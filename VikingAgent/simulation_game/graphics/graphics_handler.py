...
class GraphicsHandler:
    def __init__(self, type: GraphicsType = GraphicsType.NoGraphics, scenario: Optional[SimulationScenario] = None, UI_settings: Optional[Dict[str, bool]] = None, graphics_obj: Optional[KesslerGraphics] = None) -> None:
        """
    ...
    def update(self, ships: List[Ship], asteroids: List[Asteroid]) -> None:
        ...
        if self.type != GraphicsType.NoGraphics:
            assert self.graphics is not None
            self.graphics.update(ships, asteroids)
