...
class KesslerGame:
    ...
    def run(self, sim_scenario: SimulationScenario, controllers: List[KesslerController]) -> Tuple[Score, List[PerfDict]]:
        """
        ...
        bullets: List[Bullet] = sim_scenario.bullets()
        mines: List[Mine] = sim_scenario.mines()

        # Initialize Scoring class
        score = Score(sim_scenario)
        ...
        while stop_reason == StopReason.not_stopped:
            ...
            bullets = [bullet
                       for bullet
                       in bullets
                       if 0.0 <= bullet.position[0] <= sim_scenario.map_size[0]
                       and 0.0 <= bullet.position[1] <= sim_scenario.map_size[1]]
                    ...
                    for ship in liveships:
                        if not ship.is_respawning:
                            ...
                            if dx * dx + dy * dy <= radius_sum * radius_sum:
                                ...
                                ship.destruct(map_size=sim_scenario.map_size)
                ...
                for ship2 in liveships[i + 1:]:
                    if not ship2.is_respawning and not ship1.is_respawning:
                        ...
                        if abs(dx) <= radius_sum and abs(dy) <= radius_sum and dx * dx + dy * dy <= radius_sum * radius_sum:
                            ship1.destruct(map_size=sim_scenario.map_size)
                            ship2.destruct(map_size=sim_scenario.map_size)
            ...
            # All live ships are out of bullets and no bullets are on map
                    ...
                    and sim_scenario.stop_if_no_ammo:
                stop_reason = StopReason.out_of_bullets

            # Exceeding simulation frame limit
            elif step >= sim_scenario.frame_limit:
                # get ttc so this run can be compared to other runs that 
                # have also reached the frame limit
                ttc = ship.get_ttc(game_state)
                print("ttc:", ttc)
                
                frames_to_safespot = step
                final_ttc = ttc
                stop_reason = StopReason.frame_limit_reached
            
            # Check if the ship is in a safe spot
            ttc = ship.get_ttc(game_state)
            if ttc is None or ttc > sim_scenario.ttc_threshold:

                frames_to_safespot = step
                final_ttc = ttc
                stop_reason = StopReason.safe_spot_reached
                ...
                if not ship.is_respawning: 
                    for idx_ast, asteroid in enumerate(asteroids):
                        ...
                        if abs(dx) <= radius_sum and abs(dy) <= radius_sum and dx * dx + dy * dy <= radius_sum * radius_sum:
                            ...
                            # Add the number of frames it took to reach collision
                            frames_to_safespot = step
        ...
        final_ttc = 10 if final_ttc is None or final_ttc > 10 else final_ttc
        
        score.finalize(sim_time, stop_reason, ships, frames_to_safespot, final_ttc, ship.ownstate, game_state)
        print("num frames this ran for", step)
