...
class Heatmap:
    def __init__(self, x_boundary: tuple[float, float], y_boundary: tuple[float, float],
                resolution: float, T: float=10, max_ttc: float=11, array_size: tuple[float, float] = (250, 200)) -> None:
    ...
    def actions(self, ship_state: Dict, game_state: Dict):
        ...
        if False: # Controlls if heatmap is plotted or not
    ...
    def get_Key(asteroid: Dict, game_state: Dict):
        ...
        initialPos = (round(asteroid["position"][0]-game_state["time"]*asteroid["velocity"][0])%1000, round(asteroid["position"][1]-game_state["time"]*asteroid["velocity"][1])%800)
        #print(initialPos)
        hashKey = hash((initialPos, round(asteroid["velocity"][0]), round(asteroid["velocity"][1]), int(asteroid["size"])))
    ...
    def update_map(self, ship_state: Dict, game_state: Dict) -> None:
        ...
        for asteroid in asteroids:
            ...
            if key in old_keys and (self.computed_asteroids[key][1]-game_state["time"]) > 2.5:
                new_keys.add(key)
            ...
            #points = set((int(point[0]), int(point[1])) for point in line_points)
            points = set()
            point_and_ttc = set()

            radius = asteroid["radius"]
            grid_radius = int(np.ceil(radius*1.5 / self.resolution)) + 2  # was 4
            ...
            for i in range(len(line_points)):
                ...
                for dx in range(-grid_radius, grid_radius+1):
                    for dy in range(-grid_radius, grid_radius+1):
                        if np.sqrt(dx**2 + dy**2) > grid_radius: continue
                        ...
                        ttc = round(np.sqrt(xRel**2 + yRel**2)/grid_vel + game_state["time"], 3)
        ...
        for old_key in old_keys:
            if not old_key in new_keys:
                elems_missed = 0
                for elem in self.computed_asteroids[old_key][0]:
                    coord = elem[0]
                    ttc = elem[1]
                    try:
                        self.grid[coord[0], coord[1]]["ttcs"][1].remove(ttc)
                        if self.grid[coord[0], coord[1]]["ttcs"][0] == ttc:
                            self.update_lowest_ttc(self.grid[coord[0], coord[1]]["ttcs"], game_state)
                    except:
                        #print(f"\nCould not find the ttc to remove\n Set is: {self.grid[coord[0], coord[1]]["ttcs"][1]} \n Looking for: {ttc}")
                        elems_missed += 1
                        pass
                print(f"Missed {elems_missed} elems")
                del self.computed_asteroids[old_key]
    ...
    def update_lowest_ttc(self, entry: tuple[float, set[float]], game_state: Dict):
        lowest = self.max_ttc
        for ttc in entry[1]:
            if lowest > ttc-game_state["time"] > 0 and ttc != self.max_ttc:
                lowest = ttc
    ...
    def update_array(self, ship_state: Dict, game_state: Dict):
        ...
        for i in range(self.array_size[0]):
            for j in range(self.array_size[1]):
                    self.grid_array[i, j] = (self.grid[(i+x_low)%self.grid_size[0], (j+y_low)%self.grid_size[1]]["ttcs"][0]-game_state["time"]) 
    ...
    def weighted_gradient(self, ship_radius: float, points: int, ship_state: Dict, game_state: Dict) -> tuple[float, float]:
        ...
        #self.update_array(ship_state, game_state)
        self.apply_gaussian(sigma = 1)
        grad_x, grad_y = np.gradient(self.grid_array_gaussian)
        self.ship_ttc = self.grid_array[self.ship_position_array[0], self.ship_position_array[1]]
        grid_radius = int((ship_radius + margin) / self.resolution)
        ...
        for theta in theta_list:
            ...
            #TODO
            lowest_ttc = ttc if lowest_ttc is None or ttc < lowest_ttc else lowest_ttc
        #TODO
        self.ship_ttc = lowest_ttc
    ...
    def plot(self):
        ...
        if self.first_plot:
            self.mapArtist = plt.imshow(self.grid_array_gaussian.T, cmap='hot', interpolation='nearest', origin="lower")
