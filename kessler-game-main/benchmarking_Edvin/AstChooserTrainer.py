...
import numpy as np
import random
from AstChooserGenome import AsteroidChooserGenome as Genome
...
class NewGeneration:
    ...
    def eval_fitness(self, eval_method: str) -> list[float]:
        ...
        bestTime = np.inf
        for ind in self.pop:
            result = Game.runScenario(astChooserGenome=ind, angle = angle, graphics=False)
            newScore = result.teams[0].asteroids_hit
            if newScore >= bestScore: 
                bestScore = newScore
                bestGenome = ind
            newTime = result.sim_time
            if newTime < bestTime:
                bestTime = newTime
                bestGenome = ind
            score.append(newScore)
            accuracy.append(result.teams[0].accuracy)
            time.append(1/result.sim_time)
        ...
        print(f"\n Fastest genome: {bestGenome.state}")
