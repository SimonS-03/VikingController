...
class evasionController:
    def __init__(self, heatmap_size: tuple[float, float]=(1000, 800), heatmap_resolution: float=12):
    ...
    def actions(self, ship_state, game_state):
        ...
        elif abs(diff) < 15 and self.ship_ttc < 4:
                # brake
            if ship_state["speed"] > 240:
                thrust = -360
            elif ship_state["speed"] < 240:
                thrust = 360
        
        elif abs(diff) > 165 and self.ship_ttc < 4:
            if ship_state["speed"] > -240: # was 40
                thrust = -360 # was 240
            else:
                thrust = 360
        else:
