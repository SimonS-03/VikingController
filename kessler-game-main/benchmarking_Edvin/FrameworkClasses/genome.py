...
class Genome:
    ...
    def makeSymmetric(self):
        for gene_name in self.symmetricGenes:
    ...
    def ApplyToTrap(self, member_function: ctrl.Antecedent, names: list[str], gene_to_apply: str):
        if not self.geneNameToEdges.keys().__contains__(gene_to_apply):
            print(f"Gene to apply: {gene_to_apply}, all the genes: {self.geneNameToEdges.keys()}")
