...
class PositionForce(Forces):
    def __init__(self, ship_state: Dict, game_state: Dict, max_force_distance: float, scale_constant: float=1, radius: float=500) -> None:
    ...
    def update(self, ship_state: Dict, game_state: Dict, radius: float) -> None:
        ...
        if np.all(force_vector == np.array([0.0, 0.0])):
            self.heading = 0
        else: 
