...
class Game:
    ...
    def get_asteroids_hit(self):
        print([team.asteroids_hit for team in self.score.teams][0])
        return 
...
class NewGeneration:
    ...
    def mutate(self):
        for child in self.children:
            ...
            if mutate_idx % 2 == 0:
                child[mutate_idx], child[mutate_idx+1] = min(child[mutate_idx+delta]), max(child[mutate_idx])
            else:
                child[mutate_idx-1], child[mutate_idx] = min(child[mutate_idx+delta]), max(child[mutate_idx])
