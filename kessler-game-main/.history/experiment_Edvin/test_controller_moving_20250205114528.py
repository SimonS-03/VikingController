...
class FuzzyController(KesslerController):
    ...
    def create_moving_fis(self):
        if distance close and collision course => evade

    def threatscore(self, ship_state: Dict, game_state: Dict, param, radius):
        threatscore = []
        for idx in range(len(game_state["asteroids"])):
            if game_state["asteroids"][idx]["position"] > radius:
                threatscore.append(0)
        

    # param is list 
    """def get_dangerscore(self, ship_state: Dict, game_state: Dict, param, radius):
        ast_idx, ast_dist = self.find_asteroids_inside_radius(ship_state, game_state)
        dangerscore = [np.sqrt(game_state["asteroids"][idx]["velocity"][0]**2 + game_state["asteroids"][idx]["velocity"][1]**2) for idx in ]"""
